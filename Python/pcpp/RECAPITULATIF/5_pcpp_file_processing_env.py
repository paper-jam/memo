...
# DBMS free : mysql PostgreSQL, SqlLite
# DBMS paid : Oracle SqlServer, DB2
# SQL implements the SQL-92 standard
...
# !! : closing the connexion does not commit pending transaction


# if table exists
"CREATE TABLE IF NOT EXISTS"

# --  executemany : execute many use a LIST of TUPLES
tasks = [
    ("My first task", 1),
    ("My second task", 5),
    ("My third task", 10),
...
# ==> the ID fiel will automatically increase, as it is not specified
...
# ---------- updating data
cur = conn.cursor()
cur.execute("UPDATE tasks SET priority = ? WHERE id = ?", (20, 1))
c.commit()
...
# .fromstring() returns the root element from a string
...
# finding ALL element with iter methods, on all sublevel
...
# =>  the first line is considered to be the header
reader = csv.DictReader(csvfile)
for row in reader:
    print(row['Name'], ':', row['Phone'])

# specifying the header  in not present
...
# open file with mode 'a' for appending
